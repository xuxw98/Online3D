...
def multiview_seg_eval(gt_labels, seg_preds, label2cat, ignore_index, logger=None,
            evaluator_mode='slice_len_constant',num_slice=0,len_slice=0):
    """Semantic Segmentation  Evaluation.
    ...
    for i in range(len(gt_labels)):
        ...
        if evaluator_mode == 'slice_len_constant':
            j=1
            while j*len_slice<(len(gt_labels[i])//20000):
                timestamps.append(j*len_slice)
                j=j+1
            timestamps.append((len(gt_labels[i])//20000))
        else:
            num_slice = min((len(gt_labels[i])//20000),num_slice)
            for j in range(1,num_slice):
                timestamps.append(j*((len(gt_labels[i])//20000)//num_slice))
            timestamps.append((len(gt_labels[i])//20000))
        # online evaluation
        for j in range(len(timestamps)):
            ts = timestamps[j]
            res = fast_hist(pred_seg[(0 if j==0 else timestamps[j-1])*20000:ts*20000], gt_seg[(0 if j==0 else timestamps[j-1])*20000:ts*20000], num_classes)
