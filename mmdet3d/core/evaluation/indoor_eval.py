...
# regard each timestamp as a scene when calculate AP, AR, etc.
def hash(img_id, timestamp):
    return img_id * 300 + timestamp

#'''
def multiview_indoor_eval(gt_annos,
                dt_annos,
                metric,
                label2cat,
...
# add a new parameter
    ...
    assert len(dt_annos) == len(gt_annos)
    # first index: label second index: scene_id list element: len(ts) * bbox and score
    pred = {}  # map {class_id: pred}
    gt = {}  # map {class_id: gt}
    for img_id in range(len(dt_annos)):
        # parse detected annotations
        det_anno_all = dt_annos[img_id]
        gt_anno = gt_annos[img_id]
        timestamps = []
        ...
        if evaluator_mode == 'slice_len_constant':
            i=1
            while i*len_slice<len(gt_anno['box_masks']):
                timestamps.append(i*len_slice)
                i=i+1
            timestamps.append(len(gt_anno['box_masks']))
        else:
            num_slice = min(len(gt_anno['box_masks']),num_slice)
            for i in range(1,num_slice):
                timestamps.append(i*(len(gt_anno['box_masks'])//num_slice))
            timestamps.append(len(gt_anno['box_masks']))
        ...
        for j in range(len(timestamps)):
            ...
            for i in range(len(det_anno['labels_3d'])):
                ...
                if label not in pred:
                    pred[int(label)] = {}
                if hash(img_id, ts) not in pred[label]:
                    pred[int(label)][hash(img_id, ts)] = []
                if label not in gt:
                    gt[int(label)] = {}
                if hash(img_id, ts) not in gt[label]:
                    gt[int(label)][hash(img_id, ts)] = []
                pred[int(label)][hash(img_id, ts)].append((bbox, score))

            # parse gt annotations
            if with_box_mask:
                if j == 0:
                    box_mask_whole = np.array(gt_anno['box_masks'])[:ts, :].sum(0).astype('bool')
                else:
                    box_mask_whole = np.array(gt_anno['box_masks'])[timestamps[j-1]:ts, :].sum(0).astype('bool')
            ...
            for i in range(len(labels_3d)):
                ...
                if label not in gt:
                    gt[label] = {}
                if hash(img_id, ts) not in gt[label]:
                    gt[label][hash(img_id, ts)] = []
                gt[label][hash(img_id, ts)].append(bbox)
    
    # all sum even first online detection
    rec, prec, ap = eval_map_recall(pred, gt, metric)
    ret_dict = dict()
    header = ['classes']
    table_columns = [[label2cat[label]
...
# add a new parameter
    ...
    for img_id in range(len(dt_annos)):
        # parse detected annotations
        det_anno_all = dt_annos[img_id]
        # print("len det_anno_all")
        # print(len(det_anno_all))
        timestamps = [] if det_anno_all == [] else [len(det_anno_all) - 1]
        k = 0 if len(timestamps)==0 else timestamps[0]
        while k >= interval:
            k -= interval; timestamps.insert(0, k)

            
        # online evaluation
        for ts in timestamps:
            det_anno = det_anno_all[ts]
            for i in range(len(det_anno['labels_3d'])):
                label = det_anno['labels_3d'].numpy()[i]
                bbox = det_anno['boxes_3d'].convert_to(box_mode_3d)[i]
                score = det_anno['scores_3d'].numpy()[i]
            ...
            gt_anno = gt_annos[img_id]
            if with_box_mask:
                box_mask_whole = np.array(gt_anno['box_masks'])[:(ts+1), :].sum(0).astype('bool')
                if gt_anno['gt_num'] != 0 and sum(box_mask_whole) > 0:
                    gt_boxes = box_type_3d(
                        gt_anno['gt_boxes_upright_depth'][box_mask_whole],
                        box_dim=gt_anno['gt_boxes_upright_depth'][box_mask_whole].shape[-1],
