...
def point_sample2(img_meta,
    ...
    point_features = F.grid_sample(
        img_features,
        grid[:,:,:-memory_point_num],
        mode=mode,
        padding_mode=padding_mode,
        align_corners=align_corners)  # 1xCx1xN feats
    point_features_ori = F.grid_sample(
        img_features_ori,
        grid[:,:,-memory_point_num:],
        mode=mode,
        padding_mode=padding_mode,
    ...
    return point_features.squeeze().t(), point_features_ori.squeeze().t()


def point_project(img_meta,
                  points,
                  features,
                  proj_mat,
                  coord_type,
    ...
    camera_front_idx = pts_2d[:, 2] > 0
    pts_2d = pts_2d[camera_front_idx]
    features = features[camera_front_idx]

    # img transformation: scale -> crop -> flip
    # the image is resized by img_scale_factor
    img_coors = pts_2d[:, 0:2] * img_scale_factor  # Nx2
    img_coors -= img_crop_offset
    return img_coors[:,[1,0]], features # W H --> H W
