...
class TD3DInstanceHead_Online(BaseModule):
    ...
    def forward_train(self,
        ...
        bbox_all_list, bbox_data_list = self._merge_and_update(bbox_data_list, [(assigned_bbox_list[i][3], assigned_bbox_list[i][4], assigned_bbox_list[i][2]) for i in range(len(img_metas))], self.test_cfg, img_metas, mode='train')
            ...
            if len(bbox_pre) != 0:
                bbox_now = [(torch.cat([bboxes, bboxes2], dim=0), torch.cat([scores, scores2], dim=0), torch.cat([gt_idxs, gt_idxs2], dim=0))
                    for (bboxes, scores, gt_idxs), 
                        (bboxes2, scores2, gt_idxs2) in zip(bbox_pre, bbox_now)]
            bbox_all_list = [self._nms_merge(bbox_now[i][0], bbox_now[i][1], bbox_now[i][2], cfg, input_metas[i], mode='train')
