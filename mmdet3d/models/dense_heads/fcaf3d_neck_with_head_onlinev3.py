...
class Fcaf3DNeckWithHead_OnlineV3(BaseModule):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 init_cfg=None):
        super(Fcaf3DNeckWithHead_OnlineV3, self).__init__(init_cfg)
    ...
    def forward(self, x):
        ...
        """
        center_preds, bbox_preds, cls_preds, points, proposals = [], [], [], [], []
        inputs = x
        x = inputs[-1]
        prune_score = None
        for i in range(len(inputs) - 1, -1, -1):
            ...
            center_pred, bbox_pred, cls_pred, point, proposal, prune_score = \
        ...
        return center_preds[::-1], bbox_preds[::-1], cls_preds[::-1], \
            points[::-1], proposals[::-1]

    def forward_train(self, x, gt_bboxes, gt_labels, bbox_data_list, input_metas):
        ...
        """
        center_preds, bbox_preds, cls_preds, points, proposals = self(x)
        # boxes after NMS, before formatting
        bbox_now = self._get_data_bboxes(center_preds, bbox_preds, cls_preds, points, proposals, input_metas)
        # merge, NMS
        _, bbox_data_list = self._merge_and_update(bbox_data_list, bbox_now, input_metas, "train")
        return self._loss(center_preds, bbox_preds, cls_preds, points,
                          gt_bboxes, gt_labels, input_metas), bbox_data_list

    def forward_test(self, x, bbox_data_list, input_metas):
        ...
        bbox_now = self._get_data_bboxes(center_preds, bbox_preds, cls_preds, points, proposals, input_metas)
        # merge, NMS
        bbox_list, bbox_data_list = self._merge_and_update(bbox_data_list, bbox_now, input_metas, "test")
        return bbox_list, bbox_data_list
    
    def _merge_and_update(self, bbox_pre, bbox_now, input_metas, mode):
        ...
        if mode == "train":
            if len(bbox_pre) != 0:
                bbox_now = [(torch.cat([bboxes, bboxes2], dim=0), torch.cat([scores, scores2], dim=0), 
                            torch.cat([levels, levels2], dim=0), torch.cat([points, points2], dim=0), 
                            torch.cat([proposals, proposals2], dim=0))
                    for (bboxes, scores, levels, points, proposals), 
                        (bboxes2, scores2, levels2, points2, proposals2) in zip(bbox_pre, bbox_now)]
            bbox_all_list = [self._single_scene_multiclass_nms(bbox_now[i][0], bbox_now[i][1], 
                    bbox_now[i][2], bbox_now[i][3], bbox_now[i][4], input_metas[i], get_data=True)
                    for i in range(len(bbox_now))]
        else:
            bbox_all_list = self._merge_bbox(bbox_pre, bbox_now, input_metas)
    ...
    def _forward_single(self, x, scale):
        ...
        """
        center_pred = self.centerness_conv(x).features
        ...
        proposals = x.decomposed_features
        for i in range(len(points)):
            points[i] = points[i] * self.voxel_size

        return center_preds, bbox_preds, cls_preds, points, proposals, prune_scores
    ...
    def _get_data_bboxes_single(self, center_preds, bbox_preds, cls_preds, points, proposals,
                           input_meta):
        ...
        """
        mlvl_bboxes, mlvl_scores, mlvl_levels, mlvl_points, mlvl_proposals = [], [], [], [], []
        for i, (center_pred, bbox_pred, cls_pred, point, proposal) in enumerate(zip(
                center_preds, bbox_preds, cls_preds, points, proposals)):
            scores = cls_pred.sigmoid() * center_pred.sigmoid()
            max_scores, _ = scores.max(dim=1)
            level = torch.ones(point.shape[0], device=point.get_device())*i
            ...
            if len(scores) > self.test_cfg.nms_pre > 0:
                ...
                scores = scores[ids]
                point = point[ids]
                proposal = proposal[ids]
                level = level[ids]

            bboxes = self._bbox_pred_to_bbox(point, bbox_pred)
            mlvl_bboxes.append(bboxes)
            mlvl_scores.append(scores)
            mlvl_levels.append(level)
            mlvl_points.append(point)
            mlvl_proposals.append(proposal)

        bboxes = torch.cat(mlvl_bboxes)
        scores = torch.cat(mlvl_scores)
        points = torch.cat(mlvl_points)
        proposals = torch.cat(mlvl_proposals)
        levels = torch.cat(mlvl_levels)
        bboxes, scores, levels, points, proposals = self._single_scene_multiclass_nms2(bboxes, scores, levels, points, proposals)
        return bboxes, scores, levels, points, proposals

    def _get_data_bboxes(self, center_preds, bbox_preds, cls_preds, points, proposals,
                    input_metas):
        ...
        results = []
        for i in range(len(input_metas)):
            result = self._get_data_bboxes_single(
                center_preds=[x[i] for x in center_preds],
                bbox_preds=[x[i] for x in bbox_preds],
                cls_preds=[x[i] for x in cls_preds],
                points=[x[i] for x in points],
                proposals=[x[i] for x in proposals],
                input_meta=input_metas[i])
    ...
    def _single_scene_multiclass_nms(self, bboxes, scores, levels, points, proposals, input_meta, get_data=False):
        ...
        if get_data:
            return (nms_bboxes, nms_scores, nms_labels), \
                   (bboxes[keep_ids], scores[keep_ids], levels[keep_ids], points[keep_ids], proposals[keep_ids])
        return nms_bboxes, nms_scores, nms_labels
    
    def _single_scene_multiclass_nms2(self, bboxes, scores, levels, points, proposals):
        """Multi-class nms for a single scene.
        ...
        Returns:
            tuple[Tensor]: Predicted bboxes, scores and labels.
        """
        # there can't be too much data bboxes
        # single scene
        if len(scores) > self.test_cfg.nms_pre > 0:
            max_scores, _ = scores.max(dim=1)
            _, ids = max_scores.topk(self.test_cfg.nms_pre)
        ...
        return bboxes[keep_ids], scores[keep_ids], levels[keep_ids], points[keep_ids], proposals[keep_ids]
    
    def _merge_bbox_single(self, bbox_pre, bbox_now, input_meta):
        def keep_max(scores):
        ...
        levels_conflict = torch.cat([bbox_pre[2][~pre_index], bbox_now[2][~now_index]], dim=0)
        points_conflict = torch.cat([bbox_pre[3][~pre_index], bbox_now[3][~now_index]], dim=0)
        proposals_conflict = torch.cat([bbox_pre[4][~pre_index], bbox_now[4][~now_index]], dim=0)
        ...
        bbox_all = (torch.cat([bbox_pre[0][pre_index], bbox_now[0][now_index], bboxes_conflict], dim=0), 
                    ...
                    torch.cat([bbox_pre[2][pre_index], bbox_now[2][now_index], levels_conflict], dim=0),
                    torch.cat([bbox_pre[3][pre_index], bbox_now[3][now_index], points_conflict], dim=0),
                    torch.cat([bbox_pre[4][pre_index], bbox_now[4][now_index], proposals_conflict], dim=0))
        return self._single_scene_multiclass_nms(bbox_all[0], bbox_all[1], 
                bbox_all[2], bbox_all[3], bbox_all[4], input_meta, get_data=True)

    def _merge_bbox(self, bboxes_pre, bboxes_now, input_metas):
        if len(bboxes_pre) == 0:
            bbox_all_list = [self._single_scene_multiclass_nms(bboxes_now[i][0], bboxes_now[i][1], 
                bboxes_now[i][2], bboxes_now[i][3], bboxes_now[i][4], input_metas[i], get_data=True)
