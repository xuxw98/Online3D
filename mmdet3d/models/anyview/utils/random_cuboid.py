...
class RandomCuboid_AnyViewNew(object):
    """
    RandomCuboid augmentation from DepthContrast [https://arxiv.org/abs/2101.02691]
    We slightly modify this operation to account for object detection.
    This augmentation randomly crops a cuboid from the input and
    ...
    def __call__(self, pcs, target_boxes, per_point_labels=None):
        ...
        for _ in range(100):
            ...
            new_point_cloud = point_cloud[new_pointidx, :]
            new_point_cloud1 = point_cloud.copy()
            # so called set 0
            new_point_cloud1[~new_pointidx, :] = 0
            ...
            if self.box_filter_policy == "center":
                ...
                if per_point_labels is not None:
                    ...
                    new_per_point_labels = None
                # if we are here, all conditions are met. return boxes
                # because shape is the same
                # print('succeed')
                return new_point_cloud1.reshape(pcs.shape), new_boxes, new_per_point_labels
...
class RandomCuboid_AnyView(object):
    ...
    def __call__(self, pcs, target_boxes, per_point_labels=None):
        point_cloud = pcs.reshape(-1, pcs.shape[-1])
        # 5w and delete zero still have 4w???
        # just use 5w to simplify the problem but it is correct
