...
from .anyview import AnyViewFormer, SpatialCGNL
...
class Model3DETR_SepView(nn.Module):
    ...
    def run_encoder(self, point_clouds, trans, point_cloud_dims):
        ...
        if len(self.pre_encoder) == 2:
            pre_enc_features = pre_enc_features.view(B*T, -1, PPF, 1)
            pre_enc_features = self.pre_encoder[1](pre_enc_features)
            pre_enc_features = pre_enc_features.view(B*T, -1, PPF)
...
def build_preencoder(args):
    mlp_dims = [3 * int(args.use_color), 64, 128, 256, args.enc_dim]
    ...
    preencoder2 = SpatialCGNL(args.enc_dim, args.enc_dim // 2, use_scale=False, groups=4)
