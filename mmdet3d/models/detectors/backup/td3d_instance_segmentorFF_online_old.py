...
class TD3DInstanceSegmentorFF_Online(Base3DDetector):
    ...
    def extract_feat(self, points, img, img_metas, targets=None, mode='train'):
        ...
        if hasattr(self, 'img_memory'):
            ...
            with torch.no_grad():
                img_features = self.img_backbone(img, memory=None)['p2']
    ...
    def forward_train(self, points, modal_box, modal_label, amodal_box_mask,
        ...
        for i in range(img_metas[0]['num_frames']):
            ...
            current_feats, acc_targets = self.extract_feat(x, torch.stack(current_img, dim=0), img_metas, targets=cur_targets, mode='train')
    ...
    def simple_test(self, points, img_metas, img, *args, **kwargs):
        ...
        for i in range(len(timestamps)):
            ...
            for j in range(ts_start, ts_end):
                ...
                current_feats = self.extract_feat(x, torch.stack(current_img, dim=0), img_metas, mode='test')
