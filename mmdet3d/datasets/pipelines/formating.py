...
@PIPELINES.register_module()
class MultiViewFormatBundle3D(DefaultFormatBundle):
    """Default formatting bundle.
                ...
                if 'gt_names_3d' in results:
                    results['gt_labels_3d'] = np.array([
                        ...
                        for n in results['gt_names_3d']
                                                       ...
                                                       dtype=np.int64)
        #print('BundleAfter',end=" ")
        #print(type(results['img']))
        if 'imgs' in results:
            results['img_fields'] = ['img']
            results['img'] = results['imgs']
            del results['imgs']
        results = super(MultiViewFormatBundle3D, self).__call__(results)
        return results
